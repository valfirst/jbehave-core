...
public class PerformableTree {
    ...
    public static class PerformableStory implements Performable {
        ...
        private static final String SCENARIO_WITH_ID = "Scenario with scenarioId ";
        private static final String UPON_WHICH_CURRENT_ONE_DEPENDS_WAS = " upon which current one depends was ";
        ...
        private Optional<UUIDExceptionWrapper> causeToSkip(PerformableScenario scenario) {
            ...
            if (meta.hasProperty(DEPENDS_ON_ID)) {
                ...
                if (performDependentScenario == null) {
                    return wrap(new DependsOnNotFoundException(SCENARIO_WITH_ID + dependsOnId
                            + UPON_WHICH_CURRENT_ONE_DEPENDS_WAS + "not performed"));
                }
                if (!performDependentScenario) {
                    return wrap(new DependsOnFailedException(SCENARIO_WITH_ID + dependsOnId
                            + UPON_WHICH_CURRENT_ONE_DEPENDS_WAS + "failed"));
                }
            }
            ...
            for (Step step : steps) {
                try {
					...
					if (ignoring) {
                        ...
                        reporter.beforeStep(step.asString(keywords));
